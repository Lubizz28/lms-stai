...
export async function getClasses(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (user.role === 'mahasiswa') {
      query += ` AND cc.id IN (SELECT class_id FROM class_enrollments WHERE student_id = $1)`;
      params.push(user.id);
    } else if (user.role === 'dosen') {
      query += ` AND cc.id IN (SELECT class_id FROM class_lecturers WHERE lecturer_id = $1)`;
      params.push(user.id);
    }
